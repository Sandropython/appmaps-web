# -*- coding: utf-8 -*-
"""
Arquivo: main.py (versão comentada + integração config.json)
...
GOOGLE_API_KEY = "AIzaSyDNwadAVMmLfK7Lt-kpPmf2VbwKsh7fQXQ"  # fallback (use .env/config.json em produção)

# 🔧 NOVO: suporte a config.json
...
class AppMaps(MDApp):
    ...
    # ---------- Config (NOVO) ----------
    def ler_cfg(self):
        """Carrega config.json (com defaults)."""
        import json
        ...
        if os.path.exists(CONFIG_PATH_JSON):
            try:
                with open(CONFIG_PATH_JSON, "r", encoding="utf-8") as f:
                    data = json.load(f)
                for k in cfg:
                    cfg[k] = data.get(k, cfg[k])
            except Exception:
                pass
        return cfg

    def atualizar_cfg(self, novo_cfg: dict):
        """Permite que TelaConfig atualize as configs em memória (opcional)."""
        self.cfg = DEFAULT_CFG.copy()
        self.cfg.update(novo_cfg or {})
        print("[CFG] Atualizada em memória:", self.cfg)
    ...
    def build(self):
        ...
        # 🔧 Carrega config.json no boot
        self.cfg = self.ler_cfg()
        ...
        sm.add_widget(TelaConfig(name="tela_config"))  # integração com tela_config.py
    ...
    def selecionar_planilha(self):
        ...
        # Cria e mostra o gerenciador de arquivos
        ...
        # tenta aplicar estilo assim que abrir
        Clock.schedule_once(self._tweak_file_manager_ui, 0)

    # >>> pequena rotina que tenta aplicar estilo no MDFileManager por até ~2s
    ...
    def inicializar_googlemaps(self):
        """Cria cliente Google Maps usando prioridade: config.json → .env → fallback."""
        # Recarrega config toda vez para refletir alterações salvas na TelaConfig sem reiniciar
        self.cfg = self.ler_cfg()
        cfg_key = (getattr(self, "cfg", {}) or {}).get("GOOGLE_API_KEY")
        env_key = os.environ.get('GOOGLE_MAPS_API_KEY')
        key = cfg_key or env_key or GOOGLE_API_KEY
        if not key or key == "SUA_CHAVE_API":
            raise ValueError("API Key do Google Maps não configurada.")
        self.gmaps_lang = (getattr(self, "cfg", {}) or {}).get("GOOGLE_LANG", "pt-BR")
    ...
    def obter_dados_api_completos(self, gmaps, local, cidade):
        ...
        """
        try:
            # usa idioma configurado
            resultados = gmaps.geocode(local, language=getattr(self, "gmaps_lang", "pt-BR"))
            if not resultados:
                return None
    ...
    def buscar_latlong_rapida(self):
        ...
        """
        # Respeita opção de não usar Google
        if not getattr(self, "cfg", {}).get("USE_GOOGLE", True):
            self.show_dialog("Aviso", "A opção 'Usar Google (geocoding/rotas)' está desativada nas Configurações.")
            return
    ...
    def criar_dialogo_confirmacao(self, faltando):
        ...
        # Respeita opção de não usar Google na mensagem
        texto_extra = "" if getattr(self, "cfg", {}).get("USE_GOOGLE", True) else "\n\n⚠️ Observação: o uso da API do Google está DESATIVADO nas Configurações. Apenas dados já existentes na base serão usados."

        return MDDialog(
            MDDialogHeadlineText(text="Confirmação"),
            MDDialogSupportingText(
                text=f"{faltando} endereços não estão completos na base.\nDeseja buscar na API do Google Maps e atualizar a Base_Atual.pkl?{texto_extra}"
            ),
            MDDialogButtonContainer(
    ...
    def continuar_busca_api(self, df, base_dict):
        ...
        Fluxo principal da busca confiável:
          ...
          - Consulta API quando necessário e atualiza base (se permitido).
        ...
        # Decide se podemos chamar API
        pode_usar_api = getattr(self, "cfg", {}).get("USE_GOOGLE", True)
        ...
        for idx, row in df.iterrows():
            ...
            if local in base_dict and pd.notna(base_dict[local].get('Latitude')) and pd.notna(base_dict[local].get('Longitude')):
                ...
                if dados:
                    ...
                    # Sem API (desativada) ou sem resultado → deixa NaN
                    lat = None
                    lng = None
